...
void
CSFLSSegmentor3D< TPixel >
::setStatisticMask(typename MaskImageType::Pointer statisticMask )
{
  mp_statisticMask = statisticMask;

  TSize size = statisticMask->GetLargestPossibleRegion().GetSize();


  TIndex start = mp_statisticMask->GetLargestPossibleRegion().GetIndex();
  TIndex origin = {{0, 0, 0}};
  if (start != origin)
    {
      std::cout<<"Warrning: Force mask start to be (0, 0, 0)\n";

      TRegion region = mp_statisticMask->GetLargestPossibleRegion();
      region.SetIndex(origin);

      mp_statisticMask->SetRegions(region);
    }
